...
单测和集成测试都没覆盖到的，只有真日志能暴露：

| 现象 | 根因 | 修法 |
|---|---|---|
| 13 分钟 231 个 `web.session_started`，其中 227 个来自 `Python-urllib` | 脚本客户端不带 cookie，每次请求都被当成新访客 | 无 cookie 时按 (IP + UA) 匹配活跃会话；有 cookie 仍优先 |
| `/favicon.ico` 的 404 被记成 alert | 浏览器会主动反复请求它 | 归入静态外壳、返回 204 |
| **一行 JSON 被撕成两半；600 条事件只落盘 301 条** | 见下 | 见下 |

最后一条是**整个"两实例共写一个文件"承诺的地基塌了**，值得单独写：

1. **`FileHandle(forWritingTo:)` 不带 `O_APPEND`。** 它以 `O_WRONLY` 打开并**各自维护文件偏移量**。两个 FleetView 实例持有同一文件时，各写各的"文件末尾"，互相覆盖 —— 实测 600 条事件只剩 301 条。必须用 `open(path, O_WRONLY|O_APPEND|O_CREAT, 0600)`：只有 `O_APPEND` 才让内核把每次写定位到真正的文件尾。
2. **缓冲区把多行拼成一次写。** 攒到 8 KB 才写，而 POSIX 只保证**小于 `PIPE_BUF`(4096) 的写**是原子的 —— 8 KB 的批量写可以被内核切开，另一个 appender 正好挤进缝隙。改成**每行一次 `write(2)`**（编码器已保证每行 < 4000 字节）。

> 这两点是复合的：`O_APPEND` 保证"写到真正的末尾"，行大小 < PIPE_BUF 保证"这次写不会被切开"。**缺任一条，两实例共写就会静默损坏日志。**

补了一个直接复现该场景的测试：两个 `FileAuditSink` 并发写同一文件各 300 条，断言零撕裂、600 条一条不丢。
